...
- **`BrowserPropertyAttribute`**: Apply this to a serialized field to override the browser column display name, hide the column, show the field in a popup column, or assign a `Group`. Grouped fields are removed from the main table columns and shown within a group popup instead, using named parameters such as `DisplayName`, `HideInBrowser`, `Popup`, and `Group`.【F:Runtime/Libraries/Browsers/BrowserPropertyAttribute.cs†L1-L38】
